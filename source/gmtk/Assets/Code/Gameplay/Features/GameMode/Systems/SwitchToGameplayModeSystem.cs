...
{
    public class SwitchToGameplayModeSystem : IExecuteSystem
    {
        private readonly IGroup<GameEntity> _modeHandlers;
        private readonly IGroup<InputEntity> _inputs;
        private readonly List<GameEntity> _buffer = new List<GameEntity>(1);

        public SwitchToGameplayModeSystem(GameContext game, InputContext input)
        {
        ...
        public void Execute()
        {
            foreach (InputEntity input in _inputs)
            {
                ...
                {
                    modeHandler.isGameplayMode = true;
                    modeHandler.isBuildingMode = false;
